...
namespace QuanLyChiTieu.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Index(DateTime? tuNgay, DateTime? denNgay, string loaiGiaoDich, string tuKhoa, int trang = 1, int pageSize = 10)
        {
            ...
            // 2. GIỮ LẠI DỮ LIỆU GỐC CHO BIỂU ĐỒ
            ViewBag.DataGoc = danhSachTatCa;

            // --- LOGIC DASHBOARD (Giữ nguyên phần tính toán của bạn) ---
        ...
        //{
        //    // Đọc danh mục từ SQL và ném sang View
        //    ViewBag.DanhSachDanhMuc = await GetDanhSachDanhMucAsync();

        //    var giaoDichMoi = new GiaoDich { NgayGiaoDich = DateTime.Now };
        //    return View(giaoDichMoi);
        //}
        [HttpGet]
        public async Task<IActionResult> Create()
        {
            ...
            ViewBag.DsDanhMuc = dsDanhMuc; // Truyền List này sang View Create.cshtml
            return View();
        }
        ...
        public async Task<IActionResult> CreateBulk(List<QuanLyChiTieu.Models.GiaoDich> danhSachGiaoDich)
        {
            ...
            // 2. Lọc danh sách: Chỉ giữ lại các khoản chi có nhập số tiền > 0
            ...
            // 3. Gán tài khoản đăng nhập hiện tại cho toàn bộ danh sách hợp lệ
            foreach (var gd in danhSachThucTe)
            {
                gd.TaiKhoan = taiKhoanHienTai;
            }

            // 4. Lưu hàng loạt vào cơ sở dữ liệu qua ADO.NET SqlConnection
            string connectionString = @"Data Source=localhost\SQLEXPRESS;Initial Catalog=DB_QuanLyChiTieu;Integrated Security=True;Encrypt=False;";
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                using (SqlTransaction transaction = con.BeginTransaction())
                {
                    try
                    {
                        // Cú pháp SQL tối ưu: Kiểm tra bảng DanhMuc, nếu chưa có tên danh mục này thì tự động INSERT luôn
                        string queryDanhMuc = @"
                    ...
                    END";
                        ...
                        foreach (var gd in danhSachThucTe)
                        {
                            // BƯỚC A: KIỂM TRA VÀ TỰ ĐỘNG BỔ SUNG BẢNG DANH MỤC
